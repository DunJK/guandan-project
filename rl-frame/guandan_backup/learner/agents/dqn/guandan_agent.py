...
class MCAgent(Agent):
    ...
    def build(self) -> None:
        ...
        self.train_q = tf.compat.v1.train.RMSPropOptimizer(learning_rate=self.lr, epsilon=1e-5).minimize(self.loss)
        self.policy_model.sess.run(tf.compat.v1.global_variables_initializer())
